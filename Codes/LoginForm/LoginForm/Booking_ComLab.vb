...
Public Class Booking_ComLab
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            cmd.CommandText = "Select time from bookingsystem.time"
            dr = cmd.ExecuteReader
            cmbSection.Items.Clear()

            While dr.Read

                Dim timechoices = dr.GetString("time")
                cmbTime.Items.Add(timechoices)
        ...
        Try
            ...
            While dr.Read
                ...
                Dim RoomChoices = dr.GetString("Room_Name")
                cmbRoom.Items.Add(RoomChoices)
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Try
            cmd.Connection = con
            cmd.CommandType = CommandType.Text
            cmd.CommandText = "Select * from bookingsystem.daterequest_comlab where Date='" & dtpDate.Value.Date & "' AND Time='" & cmbTime.SelectedItem & "' AND Room='" & cmbRoom.SelectedItem & "'"
            dr = cmd.ExecuteReader
            If dr.HasRows Then
            ...
            Else
                ...
                cmd.Connection = con
                cmd.CommandType = CommandType.Text
                cmd.CommandText = "Select * from bookingsystem.approved_request where Date='" & dtpDate.Value.Date & "' AND Time='" & cmbTime.SelectedItem & "' AND Room='" & cmbRoom.SelectedItem & "'"
                dr = cmd.ExecuteReader
                If dr.HasRows Then
                    MsgBox("Another user is already using this room for this time and date. Your request has been cancelled!", MsgBoxStyle.Critical)
                    con.Close()
                Else
                    con.Close()
                    con.Open()
                    cmd.CommandText = "insert into daterequest_comlab (`Room`,`Date`,`Time`,`Grade`,`Section`,`Purpose`,`Requested_User`) VALUES ('" & cmbRoom.SelectedItem & "','" & dtpDate.Value.Date & "', '" & cmbTime.SelectedItem & "', '" & Grade & "', 
                '" & cmbSection.SelectedItem & "', '" & txtPurpose.Text & "', '" & CurrentUser & "')"
                    cmd.ExecuteNonQuery()
